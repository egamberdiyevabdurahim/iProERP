...
from buttons.admin import get_my_jobs_adm_kb
...
@router.callback_query(F.data.startswith('unfinished_works_of_mine_'), RoleFilter([1,2]))
async def my_jobs_cl(
        call: types.CallbackQuery,
        page=1,
        page_size=1,
        delete=True
):
    u = await vld(o=call, u=call.from_user, delete=delete)

    start_index = (page - 1) * page_size
    current_gadgets = await Gadget.get_all(
        ex=f"AND status = 1 AND worker={u.idn}",
        st=start_index,
        lt=page_size
    ) or []
    ...
    for gadget in current_gadgets:
        ...
        text_data += (
            ...
            f"<b>IMEI&S/N</b>: {gadget.imei1} | {gadget.imei2} | {gadget.serial_number}\n"
    ...
    try:
        await call.message.edit_text(text=text_data,
                                     parse_mode='HTML',
                                     reply_markup=await get_my_jobs_adm_kb(
                                         page=page,
                                         page_size=page_size,
                                         u=u,
